...
static int sstf_dispatch(struct request_queue *q, int force){
	...
	list_for_each(ptr, &nd->queue) {	
		requestAux = list_first_entry_or_null(ptr, struct request, queuelist);
		long long unsigned actualRequestDistance = abs(lastPos - blk_rq_pos(rq));
		long long unsigned iteratorRequestDistance = abs(lastPos - blk_rq_pos(requestAux));
		if (iteratorRequestDistance <= actualRequestDistance) {
			rq = requestAux;			
		}
	}   
	
	if (rq) {
		lastPos = blk_rq_pos(rq);
